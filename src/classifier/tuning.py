...
annotated_df = pd.read_csv('../../data/Universal_Annotation_Results_Selection.csv')
...
def hyperparameter_tuning(algo_name, X, Y, vectorize_fn, folds, three_class=False):
    ...
    if algo_name == "logreg":
        ...
        #         tuned_parameters = [{'C': [1, 1000], 'penalty': ['l2'], 'solver': ['lbfgs']}]
    ...
    for param_dict in param_object:
        ...
        for train_indices, test_indices in skf.split(X=np.zeros(len(Y)), y=Y):  # only really need Y for splitting
            ...
            if three_class:
                ...
                auprc = average_precision_score(y_test, preds_with_probs[:, 1],
                                                pos_label="POS")  # need to pass probabilities for positive class
    ...
    return round(best_f1, 3), round(corresponding_auroc, 3), round(corresponding_weighted_f1, 3), round(
        corresponding_precision, 3), round(corresponding_recall, 3), round(corresponding_accuracy, 3), round(
        corresponding_auprc, 3), corresponding_params
